...
static WiFiClient mqttNet,otaNet;static PubSubClient mq(mqttNet);static WiFiManager wm;ESPManagerClass ESPManager;static String base(){return String("espmanager/")+ESPMANAGER_DEVICE_ID;}static void cb(char*t,byte*p,unsigned int n){String s;for(unsigned i=0;i<n;i++)s+=(char)p[i];ESPManager.handleCommand(t,s);}static void report(const String&s,int p,const String&m){JsonDocument d;d["state"]=s;d["percent"]=p;d["message"]=m;String o;serializeJson(d,o);if(mq.connected())mq.publish((base()+"/ota/progress").c_str(),o.c_str(),true);}
void ESPManagerClass::begin(){Serial.begin(115200);WiFi.mode(WIFI_STA);WiFi.setAutoReconnect(true);WiFi.persistent(true);wm.setConfigPortalBlocking(false);wm.autoConnect((String("ESPManager-")+ESPMANAGER_DEVICE_ID).c_str());mq.setServer(ESPMANAGER_MQTT_HOST,ESPMANAGER_MQTT_PORT);mq.setBufferSize(2048);mq.setCallback(cb);mqttConnect();}
void ESPManagerClass::loop(){wm.process();unsigned long n=millis();if(WiFi.status()==WL_CONNECTED){lostAt=0;mqttConnect();mq.loop();if(n-lastStatus>30000)status();}else{if(!lostAt)lostAt=n;if(n-lastWifi>ESPMANAGER_WIFI_RECONNECT_INTERVAL){lastWifi=n;WiFi.reconnect();}if(n-lostAt>60000)wm.startConfigPortal((String("ESPManager-")+ESPMANAGER_DEVICE_ID).c_str());if(n-lostAt>ESPMANAGER_WIFI_RECOVERY_RESTART_AFTER)ESP.restart();}}
...
void ESPManagerClass::handleCommand(const String&t,const String&p){JsonDocument d;if(deserializeJson(d,p))return;if(String((const char*)(d["token"]|""))!=ESPMANAGER_OTA_TOKEN)return;if(t.endsWith("/cmd/restart"))ESP.restart();if(t.endsWith("/cmd/ota")){String u=d["url"]|"";report("received",0,"Kommando empfangen");
#ifdef ESP8266
...
#endif
if(r==HTTP_UPDATE_OK){report("finished",100,"Update vollständig, Neustart folgt");for(int i=0;i<8;i++){mq.loop();delay(100);}ESP.restart();}else report("failed",0,"Update fehlgeschlagen");}}
