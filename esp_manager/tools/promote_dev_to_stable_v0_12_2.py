...
dev = root / "esp_manager"
stable = root / "esp_manager"
if not dev.is_dir() or not stable.is_dir():
    raise SystemExit("esp_manager oder esp_manager fehlt")
...
for p in stable.rglob("*"):
    ...
    try:
        ...
        continue
    s = s.replace("/config/esp_manager", "/config/esp_manager")
    s = s.replace("esp_manager", "esp_manager")
    s = s.replace("ESP Manager", "ESP Manager")
    s = s.replace("ESP_MANAGER", "ESP_MANAGER")
    # Stable ingress port. Word boundaries avoid changing unrelated larger numbers.
    s = re.sub(r"(?<!\d)8099(?!\d)", "8099", s)
    p.write_text(s)
